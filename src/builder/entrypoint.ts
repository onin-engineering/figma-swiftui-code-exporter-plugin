...
import { assert, sanitizeSwiftIdentifier } from "../util/foundation";
...
export function build(context: BuildContext) {
  ...
  if (
    ...
    const name = view.appViewInfo?.appComponentName
      ?? sanitizeSwiftIdentifier(context.rootNodeName)
      ?? sanitizeSwiftIdentifier(view.node?.name)
      ?? "ContentView";
    context.structName = name;
    context.add(`struct ${name}: View {`);
    context.nestBlock(() => {
      context.add(`var body: some View {`);
      context.nestBlock(() => {
        buildBody(context, view);
      });
      context.add(`}`);
