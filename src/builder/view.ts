...
const containerTypesForComment = new Set([
  "VStack", "HStack", "ZStack", "ScrollView", "List",
  "Button", "Section", "LazyVGrid", "LazyHGrid",
]);

const magicPrefixes = ["SwiftUI::", "App::", "SFSymbols::", "Export::"];

export function buildView(context: BuildContext, view: SwiftUIViewType & View) {
  trace("#buildView", context, view);

  if (
    containerTypesForComment.has(view.type) &&
    view.node?.name &&
    !magicPrefixes.some((prefix) => view.node!.name.startsWith(prefix))
  ) {
    context.add(`// ${view.node.name}`);
  }
  ...
  if (view.type === "VStack") {
    ...
    const key = view.node?.name;
    if (key && !context.option?.isGenerateOnlyView) {
      let uniqueKey = key;
      let counter = 2;
      while (context.textStrings.has(uniqueKey) && context.textStrings.get(uniqueKey) !== view.text) {
        uniqueKey = `${key} ${counter}`;
        counter++;
      }
      context.textStrings.set(uniqueKey, view.text);
      context.add(`Text(String(localized: "${uniqueKey}"))`);
    } else {
    ...
    context.add(`ScrollView(.${view.axis}) {`);
    context.nestBlock(() => {
      view.children.forEach((e) => {
        buildBody(context, e);
      });
  ...
  if (isContainerType(view) && view.type !== "TextField") {
